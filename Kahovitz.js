...
function generateMarkdown(data) {
  // Create a combined timeline of conversations, summaries, and narrations
  const timeline = [];
  ...
  // Generate the story with narrations in flow
  let storyContent = '';
  timeline.forEach(item => {
    if (item.type === 'conversation') {
      storyContent += `### Turn ${item.turn}: ${item.speaker}\n${item.text}\n\n`;
    } else if (item.type === 'summary') {
      storyContent += `**📝 Summary (Turn ${item.turn}):** ${item.summary}\n\n`;
    } else if (item.type === 'narration') {
      storyContent += `**📖 Narration (Turn ${item.turn}):** ${item.narration}\n\n`;
    }
  });
...
app.post('/api/generate-story', async (req, res) => {
  try {
    ...
    for (let turn = 1; turn <= totalUtterances; turn++) {
      ...
      currentSession.progress = (turn / totalUtterances) * 100;
      currentSession.message = `Generating turn ${turn}/${totalUtterances}`;
      
      // Send real-time update for this turn
      currentSession.updates.push({
        type: 'update',
        speaker: charName,
        text: dialogue,
        turn: turn
    ...
    currentSession.status = 'summarizing';
    currentSession.progress = 95;
    currentSession.message = 'Generating master summary...';
    ...
    currentSession.message = 'Story generation complete!';
    currentSession.story = { data: storyData, markdown: markdown };
...
app.get('/api/generate-story-stream', (req, res) => {
  ...
  const checkSession = () => {
    ...
    if (session.updates && session.updates.length > 0) {
      ...
      session.updates = []; // Clear sent updates
    }
                ...
                try {
                    ...
                    eventSource.onmessage = function(event) {
                        ...
                        if (data.type === 'progress') {
                            ...
                            updateDiv.innerHTML = \`<strong>\${data.speaker}:</strong> \${data.text}\`;
                            document.getElementById('liveUpdates').appendChild(updateDiv);
                            document.getElementById('liveUpdates').scrollTop = document.getElementById('liveUpdates').scrollHeight;
                        } else if (data.type === 'summary') {
                            const updateDiv = document.createElement('div');
                            updateDiv.className = 'update-item';
                            updateDiv.style.borderLeftColor = '#28a745';
                            updateDiv.innerHTML = \`<strong>📝 Summary (Turn \${data.turn}):</strong> \${data.summary}\`;
                            document.getElementById('liveUpdates').appendChild(updateDiv);
                            document.getElementById('liveUpdates').scrollTop = document.getElementById('liveUpdates').scrollHeight;
                        } else if (data.type === 'narration') {
                            const updateDiv = document.createElement('div');
                            updateDiv.className = 'update-item';
                            updateDiv.style.borderLeftColor = '#ffc107';
                            updateDiv.innerHTML = \`<strong>📖 Narration (Turn \${data.turn}):</strong> \${data.narration}\`;
                            document.getElementById('liveUpdates').appendChild(updateDiv);
                            document.getElementById('liveUpdates').scrollTop = document.getElementById('liveUpdates').scrollHeight;
